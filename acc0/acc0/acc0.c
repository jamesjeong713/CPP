...
void InsertGraphicElement(struct RasterGraphic* pA)
{
	...
	// 1. if yopu malloc for string length of fileName here, 
	// you just allocate memory of the 30 boxes of the char. 
	// so that it will be used excessive memory

	printf("Insert a GraphicElement in the RasterGraphic\n");
	printf("Please enter the GraphicElement filename: ");
	scanf("%30s", &fileName);
	// 1. Therefore, you should allocate memory after the scanf which user input.
